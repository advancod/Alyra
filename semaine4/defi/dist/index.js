...
try {
  await ethereum.enable();
	...
	await contractWrite.payerDemande(IDDemande);
} catch(err) {
  console.error(err);
}
}
...
document.getElementById('getDemande').innerHTML = await contractWrite.getDemande(ID);
} catch(err) {
  console.error(err);
}
}
